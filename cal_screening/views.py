...
def main(request):
    ...
    if request.method == 'POST':
        ...
        # 实例化对象
        this_job = global_base.screening_control(id_name)
        # 创建任务路径
        ...
        # 创建文件夹
        for dir in [path_job, path_config, path_csvs, path_log, path_report]:
            os.mkdir(dir)
        # 循环上传csv小分子文件, 在前端要求上传文件, 'descriptor_file', 'test_descriptor_file''config_file' 必须上传
        for input_name in ['descriptor_file', 'test_descriptor_file', 'config_file']:
            this_job.handle_uploaded_file(request, input_name, path_csvs)
        # 把表单记录到文件中传递参数
    ...
    # 未提交表单，渲染正常页面
        ...
        return HttpResponse(render(request, 'screening/index.html', context))
