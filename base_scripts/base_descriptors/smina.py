...
def main():
    ...
    if not os.path.exists(pre_protein_file):
        ...
        cmdline += 'timeout 60 prepare_receptor4.py -r %s -o %s -A hydrogens -U nphs_lps_waters_nonstdres &&' % (
    ...
    # 多进程计算
    pool = Pool(28)
    pool.starmap(sf_py.cal_smina, ligands_xyz)
    pool.close()
    pool.join()
    # 收集数据
        ...
        if os.path.exists(log):  # 若文件存在，则读取数据
            ...
            for i in range(len(con)):
                if con[i].startswith('Term values, before weighting:'):
                    result = [x.strip() for x in con[i + 1].lstrip('##  ').split(' ')]
                    break
