...
def main():
    ...
    # 多进程计算
    pool = Pool(28)
    pool.map(sf_py.cal_ase, ligands)
    pool.close()
    pool.join()
    # 收集数据
    ...
    for ligand in ligands:
        ...
        log = '{}/{}/dock_ase.sd'.format(dst_path, lig_name)  # 日志文件
        result = []  # 初始化结果列表
        if os.path.exists(log):  # 若存在文件，读取数据
            ...
            result = []  # 初始化结果列表
            try:  # 预防错误
                for i in range(len(con)):  # 按行查找数据
                    if con[i].startswith('> <E_score2>'):  # 若数据以E_score2开头
                        result.append(con[i + 1].strip())  # 下一行数据为分数，进行读取
            finally:
