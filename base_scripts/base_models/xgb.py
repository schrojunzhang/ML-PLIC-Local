...
import os
import sys
from base_scripts.base_class.model_base import xgb_model
...
def main():
    ...
    # 低方差过滤
    train_x, test_x, feature_names = this_model.remove_same_feature(train_x=train_x, test_x=test_x,
                                                                    feature_names=feature_names)
    # 特征选择
    selector = job_form['feature_selector']  # 获取特征选择器
    selector_para = job_form['feature_selector_para']  # 选择器参数
    train_x, test_x, feature_names = this_model.feature_selection(selector, train_x=train_x, train_y=train_y,
                                                                  test_x=test_x,
                                                                  para=selector_para,
                                                                  feature_names=feature_names)  # 特征选择
    # 输出特征重要性
    ...
    if max_eval > 0 and parameter_space:  # 进行
        ...
        # 寻优
        best_parameters, trials = this_model.hyperopt_tuning(train_x=train_x, train_y=train_y, model=model,
                                                             max_evals=max_eval,
    ...
    #  获取最优模型  ####################################################################################################
    clf = this_model.get_xgb(fina_parameters)
    # 训练
